...
#         print('gt_path_L, gt_path_R', gt_path_L, gt_path_R)
#
#
...
#
#         print('**********lq_path_L, lq_path_R**********')
#         print('lq_path_L, lq_path_R', lq_path_L, lq_path_R)
#
...
#         print('img_gt_L.shape, img_gt_R.shape', img_gt_L.shape, img_gt_R.shape)
#         print('************img_lq_L.shape, img_lq_R.shape**********')
#         print('img_lq_L.shape, img_lq_R.shape', img_lq_L.shape, img_lq_R.shape)
...
class PairedStereoImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # dataroot_lq: datasets/StereoSR/lr_x4/
        self.gt_folder, self.lq_folder = opt['dataroot_gt'], opt['dataroot_lq']
    ...
    def __getitem__(self, index):
        ...
        # print('index', index)
        # print('***********gt_path************')
        # print('gt_folder:', self.gt_folder, 'gt_files[index]', self.gt_files[index])

        gt_path_L = os.path.join(self.gt_folder, self.gt_files[index], 'lr0.png')
        gt_path_R = os.path.join(self.gt_folder, self.gt_files[index], 'lr1.png')
        # if self.gt_files[index][-5] == "L":
        #     gt_path_L = os.path.join(self.gt_folder, self.gt_files[index])
        #     gt_path_R = gt_path_L
        #     gt_path_R = list(gt_path_R)
        #     gt_path_R[-5] = "R"
        #     gt_path_R = "".join(gt_path_R)
        # else:
        #     gt_path_R = os.path.join(self.gt_folder, self.gt_files[index])
        #     gt_path_L = gt_path_R
        #     gt_path_L = list(gt_path_L)
        #     gt_path_L[-5] = "L"
        #     gt_path_L = "".join(gt_path_L)
        # print('***********gt_path_L, gt_path_R************')
        # print('gt_path_L, gt_path_R', gt_path_L, gt_path_R)
        img_bytes = self.file_client.get(gt_path_L, 'gt')
        ...
        #     lq_path_R = lq_path_L
        #     lq_path_R = list(lq_path_R)
        #     lq_path_R[-5] = "R"
        #     lq_path_R = "".join(lq_path_R)
        # else:
        #     lq_path_R = os.path.join(self.lq_folder, self.lq_files[index])
        #     lq_path_L = lq_path_R
        #     lq_path_L = list(lq_path_L)
        #     lq_path_L[-5] = "L"
        #     lq_path_L = "".join(lq_path_L)
        ...
        try:
            ...
            raise Exception("lq path {} not working".format(lq_path_R))
        # print('***********img_gt_L, img_gt_R************')
        # print('img_gt_L, img_gt_R', img_gt_L.shape, img_gt_R.shape)
        # print('***********img_lq_L, img_lq_R************')
        # print('img_lq_L, img_lq_R', img_lq_L.shape, img_lq_R.shape)
        img_gt = np.concatenate([img_gt_L, img_gt_R], axis=-1)
        img_lq = np.concatenate([img_lq_L, img_lq_R], axis=-1)
        ...
        # print('lq_path', os.path.join(self.lq_folder, self.lq_files[index]))
        # print('gt_path', os.path.join(self.gt_folder, self.gt_files[index]))
        return {
            'lq': img_lq,
            'gt': img_gt,
